#!/usr/bin/env python3
"""
京剧剧本降维聚类 Pipeline（最终版）
======================================
输入：/structured/scripts/*.json + /structured/index.json
输出：/Script Dimension Reduction/drama_embeddings.json

特征工程（共235维）：
  - LDA主题向量   30维 × 权重1.2（已清洗URL噪音）
  - 角色行当分布   9维 × 权重0.8
  - 叙事特征       5维 × 权重0.6
  - 角色名二值    191维 × 权重8.0（系列分离核心信号）

分类方案：
  一级（8类）：角色名字典（+3分/个）+ 情节关键词（+1分/词），优先级消歧
  二级（35系列）：同一机制在系列层面再次匹配

降维：Supervised UMAP，以35个系列标签为监督信号
       n_neighbors=20, min_dist=0.08, target_weight=0.85, metric=cosine

包络圆：密度最优算法，R_MAX=0.28，覆盖尽量多的点
         "其他XX" 和 "市井百态" 系列不画包络圆
"""

import json, os, re, pickle, warnings
import numpy as np
...
OUT  = '/Users/dan/Desktop/VISeer/ChinaVIS/Script Dimension Reduction'
...
# 5. 合并特征
Xl = StandardScaler().fit_transform(X_lda)  * 1.2
Xr = StandardScaler().fit_transform(X_role) * 0.8
Xn = StandardScaler().fit_transform(X_narr) * 0.6
Xc = X_char * 8.0
X_combined = np.hstack([Xl, Xr, Xn, Xc])
print(f"合并特征 {X_combined.shape}")

# ── Supervised UMAP（系列级监督）─────────────────────────
le = LabelEncoder()
y = le.fit_transform(series_list)
print("Supervised UMAP...")
reducer = umap.UMAP(n_components=2, n_neighbors=20, min_dist=0.08,
                    metric='cosine', random_state=42,
                    target_metric='categorical', target_weight=0.85)
...
# ── 包络圆（密度最优，R_MAX=0.28）────────────────────────
R_MAX = 0.28
def best_circle(coords):
    arr = np.array(coords)
    candidates = list(arr[:min(len(arr),15)])
    candidates.append([arr[:,0].mean(), arr[:,1].mean()])
    if len(arr)>1:
        for i in range(min(len(arr),8)):
            for j in range(i+1,min(len(arr),8)):
                candidates.append([(arr[i,0]+arr[j,0])/2,(arr[i,1]+arr[j,1])/2])
    best_cx,best_cy,best_n = arr[:,0].mean(),arr[:,1].mean(),0
    for cand in candidates:
        cx,cy=cand[0],cand[1]
        dists=np.sqrt((arr[:,0]-cx)**2+(arr[:,1]-cy)**2)
        n=int((dists<=R_MAX).sum())
        if n>best_n: best_n,best_cx,best_cy=n,cx,cy
    dists=np.sqrt((arr[:,0]-best_cx)**2+(arr[:,1]-best_cy)**2)
    in_r=dists[dists<=R_MAX]
    r=float(in_r.max()) if len(in_r)>0 else 0.025
    return best_cx,best_cy,max(r,0.025),best_n

by_series_coords = defaultdict(list)
for i in range(len(ids)):
    by_series_coords[series_list[i]].append((float(x_n[i]),float(y_n[i])))

SERIES_CAT = {}
series_hull = {}
for sname, coords in by_series_coords.items():
    SERIES_CAT[sname] = categories[ids.index(
        next(ids[i] for i in range(len(ids)) if series_list[i]==sname))]
    if len(coords)<3: continue
    cx,cy,r,covered=best_circle(coords)
    series_hull[sname]={"cx":round(cx,4),"cy":round(cy,4),"r":round(r,4),
                        "n":len(coords),"covered":covered}
...
for i,sid in enumerate(ids):
    ...
    points.append({"id":sid,"title":s.get('title',d.get('title','')),"x":round(float(x_n[i]),4),
                   "y":round(float(y_n[i]),4),"category":categories[i],"series":series_list[i],
...
with open(f'{OUT}/drama_embeddings.json','w',encoding='utf-8') as f:
    json.dump(output,f,ensure_ascii=False,indent=2)

print(f"✓ 输出: {OUT}/drama_embeddings.json")
